...
def train_model(args, num_classes, model, ema_model, optimizer, ema_optimizer, training_criteria, train_labeled_loader, train_unlabeled_loader, cuda_available, epoch):
    ...
    for batch_idx in range(args.iterations):
        ...
        (train_unlabeled_x, train_unlabeled_x2), train_unlabeled_y = train_unlabeled
        ...
        if cuda_available:
            ...
            train_unlabeled_x, train_unlabeled_x2, train_unlabeled_y = train_unlabeled_x.cuda(), train_unlabeled_x2.cuda(), train_unlabeled_y.cuda()

        # calculate the output of the model on the unlabeled data on each augmented version of the unlabled input
        with torch.no_grad():
            train_unlabeled_y1 = model(train_unlabeled_x)
            train_unlabeled_y2 = model(train_unlabeled_x2)
            prob_unlabeled_y1 = (torch.softmax(train_unlabeled_y1, dim=1) + torch.softmax(train_unlabeled_y2, dim=1)) / 2
            prob = prob_unlabeled_y1**(1/args.temperature)
        ...
        mixed_input, mixed_target = mixup_data(train_labeled_x, train_labeled_y, train_unlabeled_x, train_targets, train_unlabeled_x2, train_targets, alpha=args.alpha)
